...
export function parseMetadata({
    ...
    return [
      {
        ...
        status: "error",
        errors: ["Could not compile the snippet. Please go back to the code editor to fix any syntax errors."],
        metadata: null,
      },
    ];
  }
  ...
  const functions = parseTreeResult.functions.map((metadata, index) => {
    ...
    const nonCapitalizedFullName =
      namespace +
      "." +
      (javascriptFunctionName.toUpperCase() === metadata.name.toUpperCase() ? javascriptFunctionName : metadata.name);
    ...
    return strictType<ICustomFunctionParseResult<IFunction>>({
      ...
      status: extras.errors.length > 0 ? "error" : solution.options.isUntrusted ? "untrusted" : "good",
      errors: [
        ...(solution.options.isUntrusted ? ["You must trust the snippet before its functions can be registered"] : []),
        ...extras.errors,
      ],
      metadata,
    });
  ...
  functions.forEach((func, index) => {
    functions.forEach((otherFunc, otherIndex) => {
      if (index !== otherIndex && func.javascriptFunctionName === otherFunc.javascriptFunctionName) {
        func.status = "error";
  ...
  functions.forEach((func, index) => {
    functions.forEach((otherFunc, otherIndex) => {
      if (index !== otherIndex && func.metadata.name.toUpperCase() === otherFunc.metadata.name.toUpperCase()) {
        func.status = "error";
