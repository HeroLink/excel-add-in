...
export default ({ solutionId, functions, code }: ICustomFunctionsIframeRunnerTypeScriptMetadata) => {
...
<body>
  <script>
    window.parent.${METHODS_EXPOSED_ON_CF_RUNNER_OUTER_FRAME.scriptRunnerOnLoad}(window, "${solutionId}");
  </script>
  ...
  <script>
    ...
    ${functions
      .map((func) => `CustomFunctionsDictionary["${func.fullId}"] = ${func.javascriptFunctionName};`)
      .join("\n  ")}
